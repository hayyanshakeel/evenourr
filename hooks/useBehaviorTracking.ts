...
// Hook for tracking cart behavior specifically
export function useCartTracking(userId?: string) {
  const { trackCartAdd, trackCartRemove, trackCartAbandon } = useBehaviorTracking({ userId });
  
  // Enhanced cart tracking with abandonment detection
  useEffect(() => {
    let cartAbandonTimer: NodeJS.Timeout;
        ...
        if (cartData) {
          try {
            ...
            if (cart.items && cart.items.length > 0) {
              const cartValue = cart.items.reduce((sum: number, item: any) => 
                sum + (item.price * item.quantity), 0
              );
              trackCartAbandon(cart.items, cartValue);
            }
          } catch (error) {
    ...
    window.addEventListener('storage', handleCartActivity);
    
    // Listen for mouse movement (user activity)
    document.addEventListener('mousemove', resetAbandonTimer);
    document.addEventListener('click', resetAbandonTimer);
    document.addEventListener('keypress', resetAbandonTimer);
    ...
    return () => {
      clearTimeout(cartAbandonTimer);
      window.removeEventListener('storage', handleCartActivity);
      document.removeEventListener('mousemove', resetAbandonTimer);
      document.removeEventListener('click', resetAbandonTimer);
      document.removeEventListener('keypress', resetAbandonTimer);
    };
  }, [trackCartAbandon]);
