...
import { redirectToCheckout } from './actions'; // Import the server action
import { useCart } from './cart-context';
import { DeleteItemButton } from './delete-item-button';
import { EditItemQuantityButton } from './edit-item-quantity-button';
import { LoadingDots } from 'components/loading-dots';

// New component for the checkout button
...
export default function CartModal() {
  ...
  // We only want to close the cart if the new quantity is less than the previous.
  useEffect(() => {
    if (isCartOpen && cart && quantityRef.current && quantityRef.current > cart.totalQuantity) {
      closeCart();
    }
    quantityRef.current = cart?.totalQuantity;
  }, [isCartOpen, cart, closeCart]);
            ...
            <div className="flex items-center justify-between">
              ...
              <button aria-label="Close cart" onClick={closeCart}>
                <div className="relative flex h-11 w-11 items-center justify-center rounded-md border border-neutral-200 text-black transition-colors">
                  ...
                  <div className="absolute right-0 top-0 -mr-2 -mt-2 h-4 w-4 rounded-full bg-blue-600 text-[11px] font-medium text-white">
                    {cart?.totalQuantity}
                  </div>
                </div>
              </button>
              ...
              <div className="flex h-full flex-col justify-between overflow-hidden p-1">
                <ul className="flex-grow overflow-auto py-4">
                  {cart.lines.map((item, i) => {
                    ...
                    const merchandiseUrl = `/product/${item.merchandise.product.handle}?${new URLSearchParams(
                ...
                {/* Replace the old button with a form that calls the server action */}
